...
<head>
    ...
    <title>Kajur Weekly Journals - {{ config('app.name', 'Kips') }}</title>
    <link rel="preconnect" href="https://fonts.bunny.net">
    <link href="https://fonts.bunny.net/css?family=instrument-sans:400,500,600,700" rel="stylesheet" />
    <style>
        :root { --bg:#0f172a; --text:#e2e8f0; --muted:#94a3b8; --border:#334155; --primary: {{ $appBranding->primary_color ?? '#2563eb' }}; --accent: {{ $appBranding->accent_color ?? '#38bdf8' }}; }
        * { box-sizing:border-box; margin:0; padding:0; }
        ...
        .field input,.field select,.field textarea { width:100%; border:1px solid var(--border); border-radius:10px; background:rgba(15,23,42,.7); color:var(--text); padding:10px 12px; font-size:.9rem; font-family:inherit; }
...
<body>
    ...
    @endphp
    <div class="app-shell">
        @include('dashboard.partials.kajur-sidebar', ['user' => $user, 'activePage' => 'weekly-journal'])

        <main class="main">
            <section class="topbar">
                <h1>Kajur Weekly Journals</h1>
                <p class="muted">Major scope: <strong>{{ strtoupper((string) $managedMajor) }}</strong></p>
            </section>

            <section class="card">
                <form method="GET" action="{{ route('dashboard.kajur.weekly-journal') }}">
                    <div class="filters">
                        <div class="field">
                        ...
                        <div class="field">
                            ...
                            <select id="major" name="major">
                                <option value="{{ $managedMajor }}">{{ $managedMajor }}</option>
                            </select>
                        </div>
                        <div class="field">
                ...
                <p class="muted" style="margin-top:8px;">Week range: {{ \Illuminate\Support\Carbon::parse($weekStart, 'Asia/Jakarta')->format('d M Y') }} - {{ \Illuminate\Support\Carbon::parse($weekEnd, 'Asia/Jakarta')->format('d M Y') }}</p>
            </section>
                        ...
                        <article class="student-card">
                            <div class="card-head">
                                <div>
                                    ...
                                    <div class="muted">NIS: {{ $student->student_nis ?? '-' }} | {{ strtoupper((string) ($student->major_name ?? '-')) }} | {{ $student->class_name ?? '-' }}</div>
                                </div>
                                <span class="badge">{{ $student->journal_status ? strtoupper($student->journal_status) : 'NO SUBMISSION' }}</span>
                            </div>
                            <div style="margin-top:8px;">
                                <div class="muted" style="margin-bottom:6px;">Pembimbing Sekolah (max 2)</div>
                                @php
                                    $assignedSupervisors = collect(data_get($supervisorsByStudent ?? [], (int) $student->student_id, []));
                                @endphp
                                @if (($schoolSupervisorAssignmentReady ?? false))
                                    @if ($assignedSupervisors->isEmpty())
                                        <div class="muted">No pembimbing assigned.</div>
                                    @else
                                        @foreach ($assignedSupervisors as $assigned)
                                            <div style="display:flex; gap:8px; align-items:center; margin-bottom:6px;">
                                                <span class="muted">{{ data_get($assigned, 'teacher_name', '-') }} (NIS: {{ data_get($assigned, 'teacher_nis', '-') }})</span>
                                                <form method="POST" action="{{ route('dashboard.kajur.student-supervisor.remove', $student->student_id) }}">
                                                    @csrf
                                                    <input type="hidden" name="teacher_id" value="{{ (int) data_get($assigned, 'teacher_id') }}">
                                                    <button class="btn" type="submit">Remove</button>
                                                </form>
                                            </div>
                                        @endforeach
                                    @endif
                                    <form method="POST" action="{{ route('dashboard.kajur.student-supervisor.assign', $student->student_id) }}" style="display:flex; gap:8px; margin-top:6px;">
                                        @csrf
                                        <select name="teacher_id" style="flex:1;" {{ $assignedSupervisors->count() >= 2 ? 'disabled' : '' }}>
                                            @foreach (($teacherOptions ?? collect()) as $teacher)
                                                <option value="{{ (int) $teacher->id }}">{{ $teacher->name }} (NIS: {{ $teacher->nis ?? '-' }})</option>
                                            @endforeach
                                        </select>
                                        <button class="btn" type="submit" {{ $assignedSupervisors->count() >= 2 ? 'disabled' : '' }}>Assign</button>
                                    </form>
                                @else
                                    <div class="muted">Run latest migrations to enable pembimbing assignment.</div>
                                @endif
                            </div>
                            @if ($student->journal_id)
                                ...
                                <form method="POST" action="{{ route('dashboard.kajur.weekly-journal.note', $student->journal_id) }}" style="margin-top:8px;">
                                    @csrf
                                    <div class="field">
                                        <label for="kajur_notes_{{ $student->journal_id }}">Update Kajur Notes</label>
                                        <textarea id="kajur_notes_{{ $student->journal_id }}" name="kajur_notes" rows="3" placeholder="Write kajur notes...">{{ old('kajur_notes', $student->kajur_notes ?? '') }}</textarea>
                                    </div>
