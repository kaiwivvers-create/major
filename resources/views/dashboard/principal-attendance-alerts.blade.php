...
<head>
    ...
    <title>Principal Attendance Alerts - {{ config('app.name', 'Kips') }}</title>
    <link rel="preconnect" href="https://fonts.bunny.net">
    <link href="https://fonts.bunny.net/css?family=instrument-sans:400,500,600,700" rel="stylesheet" />
    <style>
        :root { --bg:#0f172a; --text:#e2e8f0; --muted:#94a3b8; --border:#334155; --accent: {{ $appBranding->accent_color ?? '#38bdf8' }}; --primary: {{ $appBranding->primary_color ?? '#2563eb' }}; --critical:#ef4444; --warning:#f59e0b; }
        * { box-sizing:border-box; margin:0; padding:0; }
        ...
        .toolbar { display:grid; grid-template-columns:repeat(4,minmax(0,1fr)) auto auto; gap:10px; align-items:end; margin-top:10px; }
        ...
        .alert-item { border-radius:10px; padding:10px 12px; border:1px solid var(--border); background:rgba(15,23,42,.5); }
        .alert-item.critical { border-color:rgba(239,68,68,.65); background:rgba(127,29,29,.3); }
        .alert-item.warning { border-color:rgba(245,158,11,.6); background:rgba(120,53,15,.3); }
        ...
        .pill { display:inline-block; padding:2px 8px; border:1px solid var(--border); border-radius:999px; font-size:.78rem; font-weight:700; }
        .pill.critical { border-color:rgba(239,68,68,.65); color:#fecaca; }
        .pill.warning { border-color:rgba(245,158,11,.6); color:#fde68a; }
...
<body>
    ...
    <div class="app-shell">
        @include('dashboard.partials.principal-sidebar', ['user' => $user, 'activePage' => 'attendance-alerts'])
        ...
        <main class="main">
            <div class="container">
                <header class="topbar">
                    ...
                    <p class="muted">
                        Monitoring Date: {{ \Illuminate\Support\Carbon::parse($selectedDate, 'Asia/Jakarta')->format('d M Y') }}
                        @if ($selectedDate === $today)
                            &middot; Cutoff: {{ \Illuminate\Support\Carbon::parse($selectedDate . ' ' . $checkInCutoffTime, 'Asia/Jakarta')->format('H:i') }} WIB
                        @endif
                    </p>
                </header>

                <section class="card">
                    <form method="GET" action="{{ route('dashboard.principal.attendance-alerts') }}" class="toolbar">
                        <div class="field">
                        ...
                        <div class="field">
                            ...
                            <select id="class" name="class">
                                @foreach (($classOptions ?? collect()) as $class)
                                    <option value="{{ $class }}" {{ (string) $selectedClass === (string) $class ? 'selected' : '' }}>{{ $class }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="field">
                            <label for="risk">View</label>
                            <select id="risk" name="risk">
                                <option value="alerts" {{ $riskFilter === 'alerts' ? 'selected' : '' }}>Alerts Only</option>
                                <option value="all" {{ $riskFilter === 'all' ? 'selected' : '' }}>All Students</option>
                            </select>
                        </div>
                        <div class="field">
                            <label for="q">Search</label>
                            <input id="q" name="q" value="{{ $search ?? '' }}" placeholder="Student, NIS, class, company">
                        </div>
                        <div class="field">
                            <label for="per_page">Rows</label>
                            <select id="per_page" name="per_page">
                                @foreach (($perPageOptions ?? [20, 50, 100]) as $pp)
                                    <option value="{{ $pp }}" {{ (int) ($perPage ?? 20) === (int) $pp ? 'selected' : '' }}>{{ $pp }}</option>
                                @endforeach
                            </select>
                        </div>
                        <button class="btn" type="submit">Apply</button>
                        <a class="btn secondary" href="{{ route('dashboard.principal.attendance-alerts') }}">Reset</a>
                    </form>
                </section>
                ...
                <section class="card">
                    ...
                    <div class="stats-grid">
                        <div class="stat"><span class="muted">Students in Scope</span><strong>{{ (int) data_get($totals, 'students', 0) }}</strong></div>
                        <div class="stat"><span class="muted">Total Alerts</span><strong>{{ (int) data_get($totals, 'alerts', 0) }}</strong></div>
                        <div class="stat"><span class="muted">Critical</span><strong>{{ (int) data_get($totals, 'critical', 0) }}</strong></div>
                        <div class="stat"><span class="muted">Warnings</span><strong>{{ (int) data_get($totals, 'warnings', 0) }}</strong></div>
                        <div class="stat"><span class="muted">Missed Check-in</span><strong>{{ (int) data_get($totals, 'missed_checkin', 0) }}</strong></div>
                        <div class="stat"><span class="muted">Pending Excuse</span><strong>{{ (int) data_get($totals, 'pending_excuse', 0) }}</strong></div>
                        <div class="stat"><span class="muted">Late Check-in</span><strong>{{ (int) data_get($totals, 'late', 0) }}</strong></div>
                        <div class="stat"><span class="muted">Checked In/Out</span><strong>{{ (int) data_get($totals, 'checked_in', 0) }}</strong></div>
                    </div>
                    @if (($escalationNotes ?? collect())->isNotEmpty())
                        <div class="alert-list">
                            @foreach (($escalationNotes ?? collect()) as $note)
                                @php
                                    $isCritical = str_contains(strtolower((string) $note), 'critical risk');
                                    $isWarning = !$isCritical && (str_contains(strtolower((string) $note), 'follow-up') || str_contains(strtolower((string) $note), 'cutoff'));
                                @endphp
                                <div class="alert-item {{ $isCritical ? 'critical' : ($isWarning ? 'warning' : '') }}">{{ $note }}</div>
                            @endforeach
                        </div>
                    @endif
                </section>
                ...
                <section class="grid">
                    <article class="card">
                        ...
                        <table>
                            ...
                            <tbody>
                                @forelse (($majorBreakdown ?? collect()) as $row)
                                    <tr>
                                        <td>{{ data_get($row, 'major', '-') }}</td>
                                        <td>{{ (int) data_get($row, 'total', 0) }}</td>
                                        <td>{{ (int) data_get($row, 'alerts', 0) }}</td>
                                        <td>{{ (int) data_get($row, 'critical', 0) }}</td>
                                        <td>{{ (int) data_get($row, 'warning', 0) }}</td>
                                        <td>{{ number_format((float) data_get($row, 'risk_rate', 0), 1) }}%</td>
                                    </tr>
                                @empty
                    ...
                    <article class="card">
                        <h2>Recent Alert Notifications</h2>
                        <p class="muted">Stored summaries for principal account.</p>
                        <table>
                            <thead>
                                <tr>
                                    <th>Date</th>
                            ...
                            <tbody>
                                @forelse (($recentAlerts ?? collect()) as $alert)
                                    <tr>
                                        <td>{{ \Illuminate\Support\Carbon::parse($alert->alert_date, 'Asia/Jakarta')->format('d M Y') }}</td>
                                        <td>{{ str_replace('_', ' ', strtoupper((string) ($alert->alert_type ?? '-'))) }}</td>
                                        <td>{{ $alert->message }}</td>
                                    </tr>
                                @empty
                                    <tr><td colspan="3">No stored attendance alert notifications yet.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </article>
                ...
                <section class="card">
                    <h2>Student Alert Table</h2>
                    <p class="muted">{{ (int) (($alertsPage ?? collect())->total() ?? 0) }} row(s) match current filter.</p>
                    <table>
                        <thead>
                            <tr>
                                <th>Student</th>
                        ...
                        <tbody>
                            @forelse (($alertsPage ?? collect()) as $row)
                                <tr>
                                    ...
                                    <td>
                                        @if (!empty($row->check_in_at))
                                            {{ \Illuminate\Support\Carbon::parse($row->check_in_at, 'Asia/Jakarta')->format('H:i') }} WIB
                                            @if ((int) ($row->late_minutes ?? 0) > 0)
                                                <br><span class="muted">Late {{ (int) $row->late_minutes }} min</span>
                                            @endif
                                        @else
                                            -
                                        @endif
                                    ...
                                    <td>
                                        @php
                                            $level = (string) ($row->alert_level ?? 'none');
                                        @endphp
                                        <span class="pill {{ $level }}">{{ strtoupper((string) ($row->alert_type ?? 'none')) }}</span>
                                    </td>
                                    <td>{{ $row->absence_reason ?? '-' }}</td>
                                </tr>
                            @empty
                                <tr><td colspan="9">No rows found for this filter.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                    @if (($alertsPage ?? null) && method_exists($alertsPage, 'links'))
                        <div class="pagination-wrap">{{ $alertsPage->links() }}</div>
                    @endif
                </section>
            </div>
        </main>
