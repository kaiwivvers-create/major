...
<head>
    ...
    <style>
        :root { --bg:#0f172a; --text:#e2e8f0; --muted:#94a3b8; --border:#334155; --primary: {{ $appBranding->primary_color ?? '#2563eb' }}; }
        * { box-sizing:border-box; margin:0; padding:0; }
        ...
        .recent-requests { display:grid; gap:6px; min-width:260px; }
        .recent-request { border:1px solid var(--border); border-radius:8px; background:rgba(15,23,42,.42); padding:7px; }
        .recent-request-head { display:flex; align-items:center; justify-content:space-between; gap:8px; margin-bottom:4px; }
        .request-type { font-size:.78rem; font-weight:700; letter-spacing:.03em; text-transform:uppercase; }
        .request-status { display:inline-block; border-radius:999px; padding:2px 8px; font-size:.72rem; font-weight:700; text-transform:uppercase; border:1px solid var(--border); }
        .request-status.pending { border-color:rgba(245,158,11,.6); color:#fde68a; background:rgba(120,53,15,.35); }
        .request-status.approved { border-color:rgba(34,197,94,.55); color:#bbf7d0; background:rgba(20,83,45,.35); }
        .request-status.rejected { border-color:rgba(248,113,113,.6); color:#fecaca; background:rgba(127,29,29,.4); }
        .request-note { margin-top:3px; }
...
<body>
    ...
    @endphp
    <div class="app-shell">
        @include('dashboard.partials.kajur-sidebar', ['user' => $user, 'activePage' => 'absence-report'])
        <main class="main">
            <header class="topbar">
                <div>
                    <h1>Attendance Rate Report</h1>
            ...
            <section class="card">
                ...
                <div class="stats">
                    ...
                    <div class="stat"><div class="muted">Attendance Rate</div><div class="v">{{ number_format((float) ($summary['attendance_rate'] ?? 0), 2) }}%</div></div>
                </div>
            </section>
                    ...
                    <table>
                        <thead>
                            <tr>
                                ...
                                <th>Recent Sick/Permit Requests</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse (($rows ?? collect()) as $row)
                                <tr>
                                    ...
                                    <td>{{ number_format((float) $row->attendance_rate, 2) }}%</td>
                                    <td>
                                        @php
                                            $recentRequests = collect($row->recent_excuse_requests ?? []);
                                        @endphp
                                        @if ($recentRequests->isEmpty())
                                            <span class="muted">No request in selected period.</span>
                                        @else
                                            <div class="recent-requests">
                                                @foreach ($recentRequests as $request)
                                                    @php
                                                        $statusRaw = strtolower(trim((string) ($request->status ?? 'pending')));
                                                        $statusClass = in_array($statusRaw, ['pending', 'approved', 'rejected'], true) ? $statusRaw : 'pending';
                                                        $statusLabel = strtoupper($statusClass);
                                                        $typeLabel = strtoupper((string) ($request->absence_type ?? '-'));
                                                        $requestDate = !empty($request->attendance_date)
                                                            ? \Illuminate\Support\Carbon::parse($request->attendance_date, 'Asia/Jakarta')->format('d M Y')
                                                            : '-';
                                                        $reviewedAt = !empty($request->reviewed_at)
                                                            ? \Illuminate\Support\Carbon::parse($request->reviewed_at, 'Asia/Jakarta')->format('d M Y H:i')
                                                            : null;
                                                    @endphp
                                                    <div class="recent-request">
                                                        <div class="recent-request-head">
                                                            <span class="request-type">{{ $typeLabel }} · {{ $requestDate }}</span>
                                                            <span class="request-status {{ $statusClass }}">{{ $statusLabel }}</span>
                                                        </div>
                                                        @if ($statusClass === 'approved')
                                                            <div class="muted">Approved by {{ $request->reviewed_by_name ?? 'Kajur' }}{{ $reviewedAt ? ' · ' . $reviewedAt : '' }}</div>
                                                        @elseif ($statusClass === 'rejected')
                                                            <div class="muted">Rejected by {{ $request->reviewed_by_name ?? 'Kajur' }}{{ $reviewedAt ? ' · ' . $reviewedAt : '' }}</div>
                                                            @if (!empty($request->rejection_notes))
                                                                <div class="muted request-note">Note: {{ \Illuminate\Support\Str::limit((string) $request->rejection_notes, 120) }}</div>
                                                            @endif
                                                        @else
                                                            <div class="muted">Waiting for Kajur review</div>
                                                        @endif
                                                    </div>
                                                @endforeach
                                            </div>
                            ...
                            @empty
                                <tr><td colspan="10">No students in this scope.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
