<style>
    ...
    .profile-modal-field input[type="file"] {
        width: 100%;
    ...
    .profile-modal-field input[type="file"]::file-selector-button {
    ...
    .profile-modal-field input[type="file"]:hover {
    ...
    .profile-modal-panel {
        ...
        background:
            radial-gradient(120% 140% at 100% 0%, rgba(56, 189, 248, 0.16), transparent 55%),
            linear-gradient(160deg, rgba(30, 41, 59, 0.96), rgba(15, 23, 42, 0.96));
        box-shadow: 0 26px 46px rgba(2, 6, 23, 0.58);
    }
    ...
    .profile-modal-field select {
        ...
        background: linear-gradient(160deg, rgba(15, 23, 42, 0.9), rgba(2, 6, 23, 0.92));
    }
...
@php
    $profileModalShouldOpen = ($openProfileModal ?? false)
        || $errors->has('name')
        || $errors->has('nis')
        || $errors->has('password')
        || $errors->has('avatar_crop_data');
    $profileNotifications = function_exists('attendance_fetch_recent_alerts')
        ? attendance_fetch_recent_alerts($user, 8, true)
        : collect();
    $profileUnreadCount = function_exists('attendance_unread_alert_count')
        ? attendance_unread_alert_count($user)
        : 0;
@endphp
<div class="profile-modal-backdrop {{ $profileModalShouldOpen ? 'open' : '' }}" id="profile-modal-backdrop" aria-hidden="{{ $profileModalShouldOpen ? 'false' : 'true' }}">
    <div class="profile-modal-panel" role="dialog" aria-modal="true" aria-labelledby="profile-modal-title">
        ...
        <section class="notif-wrap">
            <div class="notif-head">
                <h4>Notifications ({{ (int) $profileUnreadCount }} unread)</h4>
                @if ($profileUnreadCount > 0)
            ...
            <div class="notif-list">
                @forelse ($profileNotifications as $notif)
                    @php
                        $isUnread = !(bool) data_get($notif, 'is_read', false);
                    @endphp
                    <article class="notif-item {{ $isUnread ? 'unread' : '' }}">
                        <div class="notif-title">
                            <strong>{{ str_replace('_', ' ', strtoupper((string) data_get($notif, 'alert_type', '-'))) }}</strong>
                            <span class="notif-date">{{ \Illuminate\Support\Carbon::parse((string) data_get($notif, 'alert_date', now('Asia/Jakarta')->toDateString()), 'Asia/Jakarta')->format('d M Y') }}</span>
                        </div>
                        <div class="muted">{{ data_get($notif, 'message', '-') }}</div>
                        @if ($isUnread)
                            <div class="notif-actions">
                                <form method="POST" action="{{ route('dashboard.notifications.read', (int) data_get($notif, 'id')) }}">
                                    @csrf
