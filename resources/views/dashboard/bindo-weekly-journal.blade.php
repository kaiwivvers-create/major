...
<head>
    ...
    <title>Guru Bindo Weekly Notes - {{ config('app.name', 'Kips') }}</title>
                    ...
                    <tr>
                        ...
                        <td>
                            <form method="POST" action="{{ route('dashboard.bindo.weekly-journal.note', $row->id) }}">
                                @csrf
                                <textarea name="bindo_notes" placeholder="Write language/format notes...">{{ old('bindo_notes', $row->bindo_notes ?? '') }}</textarea>
