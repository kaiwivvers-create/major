...
<body>
    ...
    @endphp
    <div class="app-shell">
        @include('dashboard.partials.super-admin-sidebar', ['user' => $user, 'activePage' => 'permissions'])
                        ...
                        <table class="permission-table">
                            ...
                            <tbody>
                                @php
                                    ...
                                    $permissionActionsDisabled = (($selectedRole ?? '') === \App\Models\User::ROLE_SUPER_ADMIN || !($rolePermissionsStorageReady ?? false));
                                @endphp
                                @foreach ($roleOrder as $roleKey)
                                    ...
                                    @foreach ($modulesInGroup as $moduleKey => $moduleMeta)
                                        ...
                                        <tr>
                                            ...
                                            <td class="center"><input class="perm-checkbox" type="checkbox" name="permissions[{{ $moduleKey }}][view]" value="1" {{ data_get($targetPermissions, "{$moduleKey}.view") ? 'checked' : '' }} {{ $permissionActionsDisabled ? 'disabled' : '' }}></td>
                                            <td class="center"><input class="perm-checkbox" type="checkbox" name="permissions[{{ $moduleKey }}][create]" value="1" {{ data_get($targetPermissions, "{$moduleKey}.create") ? 'checked' : '' }} {{ $permissionActionsDisabled ? 'disabled' : '' }}></td>
                                            <td class="center"><input class="perm-checkbox" type="checkbox" name="permissions[{{ $moduleKey }}][update]" value="1" {{ data_get($targetPermissions, "{$moduleKey}.update") ? 'checked' : '' }} {{ $permissionActionsDisabled ? 'disabled' : '' }}></td>
                                            <td class="center"><input class="perm-checkbox" type="checkbox" name="permissions[{{ $moduleKey }}][delete]" value="1" {{ data_get($targetPermissions, "{$moduleKey}.delete") ? 'checked' : '' }} {{ $permissionActionsDisabled ? 'disabled' : '' }}></td>
                                        </tr>
                                    @endforeach
                                @endforeach
                            </tbody>
