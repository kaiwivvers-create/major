...
<head>
    ...
    <style>
        :root { --bg:#0f172a; --surface:#1e293b; --primary: {{ $appBranding->primary_color ?? '#2563eb' }}; --accent: {{ $appBranding->accent_color ?? '#38bdf8' }}; --text:#e2e8f0; --muted:#94a3b8; --border:#334155; }
...
@php
    $user = auth()->user();
    $canViewMentorDashboard = function_exists('user_can_access') ? user_can_access($user, 'mentor_weekly_journal', 'view') : true;
    $canViewMentorCompanySettings = function_exists('user_can_access') ? user_can_access($user, 'mentor_company_settings', 'view') : true;
...
<div class="app-shell">
    <aside class="sidebar">
        @include('dashboard.partials.sidebar-brand', ['suffix' => 'Mentor'])
        <nav class="sidebar-nav" aria-label="Mentor menu">
            ...
            @endif
            @if ($canViewMentorCompanySettings)
    ...
    <main class="main">
        ...
        <section class="card" style="margin-top:20px;">
            <div class="card-header">
                <h2>Logo Preview Across Application</h2>
                <p>Your company logo will appear in these locations once saved:</p>
            </div>
            <div style="padding:20px;">
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:20px;">
                    <!-- Landing Page Preview -->
                    <div style="border:1px solid var(--border); border-radius:12px; padding:16px; background:rgba(15,23,42,.5);">
                        <p style="font-size:0.9rem; font-weight:600; margin-bottom:12px; color:var(--muted);">Landing Page (Company Carousel)</p>
                        <div style="border:1px solid rgba(56,189,248,.3); border-radius:10px; padding:12px; background:rgba(30,41,59,.7); min-height:140px; display:flex; flex-direction:column; gap:8px;">
                            <div style="width:100%; height:80px; border:1px solid var(--border); border-radius:8px; background:rgba(15,23,42,.7); display:grid; place-items:center; overflow:hidden;">
                                @php
                                    $logoUrl = data_get($companyProfile, 'logo_url');
                                    $logoSource = !empty($logoUrl)
                                        ? (\Illuminate\Support\Str::startsWith($logoUrl, ['http://', 'https://'])
                                ...
                                @endphp
                                @if (!empty($logoSource))
                                    <img src="{{ $logoSource }}" alt="Company logo" style="width:100%; height:100%; object-fit:cover;">
                                @else
                                    <span style="color:var(--muted); font-size:0.8rem;">No logo yet</span>
                                @endif
                            </div>
                            <p style="font-size:0.8rem; color:var(--muted); margin:0;">{{ $mentorCompanyName ?: 'Company Name' }}</p>
                        </div>
                    </div>

                    <!-- Companies List Preview -->
                    <div style="border:1px solid var(--border); border-radius:12px; padding:16px; background:rgba(15,23,42,.5);">
                        <p style="font-size:0.9rem; font-weight:600; margin-bottom:12px; color:var(--muted);">Companies Page (Admin Views)</p>
                        <div style="border:1px solid rgba(56,189,248,.3); border-radius:10px; padding:12px; background:rgba(30,41,59,.7);">
                            <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px;">
                                <div style="border:1px solid var(--border); border-radius:8px; padding:10px; background:rgba(15,23,42,.7); text-align:center;">
                                    <div style="width:100%; height:60px; border:1px solid rgba(56,189,248,.3); border-radius:6px; background:rgba(15,23,42,.9); display:grid; place-items:center; overflow:hidden; margin-bottom:8px;">
                                        @if (!empty($logoSource))
                                            <img src="{{ $logoSource }}" alt="Company logo" style="width:100%; height:100%; object-fit:cover;">
                                        @else
                                            <span style="color:var(--muted); font-size:0.7rem;">Logo</span>
                                        @endif
                                    </div>
                                    <p style="font-size:0.75rem; color:var(--muted); margin:0; word-break:break-word;">{{ $mentorCompanyName ?: 'Company' }}</p>
                                </div>
                                <div style="border:1px solid var(--border); border-radius:8px; padding:10px; background:rgba(15,23,42,.7); text-align:center; opacity:0.5;">
                                    <div style="width:100%; height:60px; border:1px solid rgba(56,189,248,.3); border-radius:6px; background:rgba(15,23,42,.9); display:grid; place-items:center; overflow:hidden; margin-bottom:8px;">
                                        <span style="color:var(--muted); font-size:0.7rem;">Logo</span>
                                    </div>
                                    <p style="font-size:0.75rem; color:var(--muted); margin:0;">Other Company</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div style="margin-top:16px; padding:12px; border-radius:10px; background:rgba(56,189,248,.1); border:1px solid rgba(56,189,248,.3);">
                    <p style="font-size:0.85rem; color:#cbd5e1; margin:0; line-height:1.5;">ℹ️ Your logo will be visible to all users on the landing page company carousel, principal's partner companies dashboard, and super admin company management pages. Changes take effect immediately after saving.</p>
                </div>
            </div>
        </section>
    </main>
