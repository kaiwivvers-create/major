...
<head>
    ...
    <title>All Check-ins - Super Admin - {{ config('app.name', 'Kips') }}</title>
    ...
    <style>
        ...
        * { box-sizing:border-box; margin:0; padding:0; }
        ...
        .badge.checked-in { border-color:rgba(56,189,248,.6); color:#bae6fd; }
        .badge.checked-out { border-color:rgba(34,197,94,.6); color:#bbf7d0; }
        .badge.no-checkin { border-color:rgba(248,113,113,.5); color:#fecaca; }
        .meta-grid { margin-top:8px; display:grid; grid-template-columns:repeat(3,minmax(0,1fr)); gap:8px; }
        .meta-box { border:1px solid var(--border); border-radius:10px; background:rgba(30,41,59,.4); padding:8px; font-size:.88rem; }
        ...
        @media (max-width:1100px){ .filters{grid-template-columns:1fr 1fr;} .app-shell{grid-template-columns:1fr;} .sidebar{position:static; height:auto;} .main{padding-top:0;} .meta-grid{grid-template-columns:1fr;} }
    </style>
</head>
<body>
    @php
    ...
    @endphp
    <div class="app-shell">
        @include('dashboard.partials.super-admin-sidebar', ['user' => $user, 'activePage' => 'checkins'])

        <main class="main">
            <section class="topbar">
                <h1>All Student Check-ins</h1>
                <p class="muted">Search and filter by major, class, status, and date.</p>
            </section>

            <section class="card">
                <form method="GET" action="{{ route('dashboard.super-admin.checkins') }}">
                    <div class="filters">
                        <div class="field">
                        ...
                        <div class="field">
                            ...
                            <select id="status" name="status">
                                @php $statusOptions = ['all' => 'All', 'checked_in' => 'Checked In', 'checked_out' => 'Checked Out', 'no_checkin' => 'No Check-in']; @endphp
            ...
            <section class="card">
                <div class="cards">
                    @forelse (($students ?? collect()) as $student)
                        @php
                            $badgeClass = 'no-checkin';
                            if ($student->attendance_state === 'Checked In') $badgeClass = 'checked-in';
                            if ($student->attendance_state === 'Checked Out') $badgeClass = 'checked-out';
                        @endphp
                        <article class="student-card">
                            <div class="card-head">
                                ...
                                <span class="badge {{ $badgeClass }}">{{ $student->attendance_state }}</span>
                            </div>
                            <div class="meta-grid">
                                <div class="meta-box">
                                    <div class="muted">Check-in</div>
                                    <div>{{ $student->check_in_at ? \Illuminate\Support\Carbon::parse($student->check_in_at, 'Asia/Jakarta')->format('d M Y H:i') . ' WIB' : '-' }}</div>
                                </div>
                                <div class="meta-box">
                                    <div class="muted">Check-out</div>
                                    <div>{{ $student->check_out_at ? \Illuminate\Support\Carbon::parse($student->check_out_at, 'Asia/Jakarta')->format('d M Y H:i') . ' WIB' : '-' }}</div>
                                </div>
                                <div class="meta-box">
                            ...
                            <div class="muted" style="margin-top:8px;">Company: {{ $student->company_name ?? '-' }}</div>
                            @if (!empty($student->photo_url))
                                <div style="margin-top:8px;"><a class="muted" href="{{ $student->photo_url }}" target="_blank" rel="noopener noreferrer">Open check-in photo</a></div>
                            @endif
                        </article>
                    @empty
