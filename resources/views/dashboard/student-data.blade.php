...
<body>
    @php
        ...
        $contactDirectoryIsComplete = filled(data_get($profile, 'pkl_place_name'))
    ...
    <div class="app-shell">
        ...
        <main class="content page-drift-up">
            <header class="topbar {{ !$profileIsComplete ? 'incomplete-glow' : '' }}">
                <h1>Student Data</h1>
                <p class="muted">Student biodata can only be edited by admin. You can view your data here.</p>
                @if (!$profileIsComplete)
                    <div class="warning-banner incomplete-glow">
                        Your student data is not complete yet. Please contact admin to complete the required fields.
                    </div>
                @endif
            </header>
                ...
                <form method="POST" action="{{ route('dashboard.student.data.save') }}">
                    ...
                    <div class="form-grid">
                        ...
                        <div class="field">
                            ...
                            <input id="class_name" name="class_name" type="text" value="{{ old('class_name', $profile->class_name ?? '') }}" placeholder="Example: XI RPL 1">
                        </div>
                    ...
                    <div class="actions">
                        <button class="btn" type="button" disabled>Admin Only</button>
                    </div>
                </form>
            </section>

            <section class="card {{ !$contactDirectoryIsComplete ? 'incomplete-glow' : '' }}">
                <h2 style="font-size:1rem;">Contact Directory Data</h2>
                <p class="muted" style="margin-top:6px;">You can update contact info used by your teacher in Contact Directory.</p>
                @if (!$contactDirectoryIsComplete)
                    <div class="warning-banner incomplete-glow" style="margin-bottom:10px;">
                        Your Contact Directory data is incomplete. Please fill all required contact fields below.
                    </div>
                @endif

                <form method="POST" action="{{ route('dashboard.student.contact-directory.update') }}">
                    @csrf
                    <div class="form-grid">
                        <div class="field">
                            <label for="contact_pkl_place_name">PKL Place *</label>
                            <input id="contact_pkl_place_name" name="pkl_place_name" type="text" value="{{ old('pkl_place_name', $profile->pkl_place_name ?? '') }}" required>
                        </div>

                        <div class="field">
                            <label for="contact_mentor_teacher_name">Industry Mentor Contact Name *</label>
                            <input id="contact_mentor_teacher_name" name="mentor_teacher_name" type="text" value="{{ old('mentor_teacher_name', $profile->mentor_teacher_name ?? '') }}" required>
                        </div>

                        <div class="field full">
                            <label for="contact_pkl_place_address">PKL Place Address *</label>
                            <textarea id="contact_pkl_place_address" name="pkl_place_address" required>{{ old('pkl_place_address', $profile->pkl_place_address ?? '') }}</textarea>
                        </div>

                        <div class="field">
                            <label for="contact_pkl_place_phone">PKL Contact Phone *</label>
                            <input id="contact_pkl_place_phone" name="pkl_place_phone" type="text" value="{{ old('pkl_place_phone', $profile->pkl_place_phone ?? '') }}" required>
                        </div>
                    </div>

                    <div class="actions">
                        <button class="btn" type="submit">Save Contact Data</button>
                    </div>
                </form>
            </section>
        </main>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            selects.forEach(select => {
                new Choices(select, {
                    ...
                    shouldSort: false,
                    classNames: {
                        containerOuter: 'choices kips-choices',
                    }
                });
            });
        });
