    ...
    <style>
        ...
        * {
        ...
        .checkin-box {
            ...
            padding: 12px;
            background:
                linear-gradient(160deg, rgba(15, 23, 42, 0.88), rgba(2, 6, 23, 0.88));
...
@endphp
            ...
            <section class="grid">
                <article class="card">
                    ...
                    <div class="today-stat">
                        ...
                        <div><strong>Address:</strong> {{ $todayAttendance->location_address ?? '-' }}</div>
                    ...
                    <div class="history-grid">
                        @foreach ($previousDays as $row)
                            <button
                                ...
                                data-address="{{ $row->location_address ?? '-' }}"
    ...
    <div class="modal-backdrop" id="checkin-modal-backdrop" aria-hidden="true">
        <div class="profile-modal checkin-modal" role="dialog" aria-modal="true" aria-labelledby="checkin-modal-title">
            ...
            <form method="POST" action="{{ route('dashboard.student.check-in') }}" id="checkin-form">
                ...
                <input type="hidden" name="location_address" id="checkin-location-address">
    ...
    <script>
        ...
        (() => {
            ...
            if (!cards.length || !backdrop || !closeBtn || !dateEl || !statusEl || !checkinEl || !checkoutEl || !ipEl || !coordinatesEl || !addressEl || !photoEl || !photoEmptyEl) return;
            ...
            cards.forEach((card) => {
                card.addEventListener('click', () => {
                    const { date, status, checkin, checkout, ip, lat, lng, address, photo } = card.dataset;
        ...
        (() => {
            ...
            const addressInput = document.getElementById('checkin-location-address');
            ...
            if (!closeBtn || !backdrop || !form || !latInput || !lngInput || !addressInput || !selfieInput || !locationStatus || !selfieStatus || !submitBtn || !getLocationBtn || !startCameraBtn || !captureBtn || !retakeBtn || !video || !preview || !canvas) return;
            ...
            const getLocation = () => {
                ...
                navigator.geolocation.getCurrentPosition(
                    async (position) => {
                        ...
                        locationStatus.textContent = `Location captured (${lat.toFixed(5)}, ${lng.toFixed(5)}). Resolving address...`;
                        try {
                            ...
                            if (response.ok) {
                                ...
                                const pretty = String(payload?.display_name || '').trim();
                                if (pretty !== '') {
                                    addressInput.value = pretty;
                                    locationStatus.textContent = `Location captured (${lat.toFixed(5)}, ${lng.toFixed(5)}). Address: ${pretty}`;
                                } else {
                                    locationStatus.textContent = `Location captured (${lat.toFixed(5)}, ${lng.toFixed(5)}). Address not available.`;
                                }
                            } else {
                                locationStatus.textContent = `Location captured (${lat.toFixed(5)}, ${lng.toFixed(5)}). Address lookup failed.`;
                            }
                        } catch (error) {
                            locationStatus.textContent = `Location captured (${lat.toFixed(5)}, ${lng.toFixed(5)}). Address lookup failed.`;
                        }
