...
<head>
    ...
    <title>All Completion Bars - Super Admin - {{ config('app.name', 'Kips') }}</title>
    <link rel="preconnect" href="https://fonts.bunny.net">
    <link href="https://fonts.bunny.net/css?family=instrument-sans:400,500,600,700" rel="stylesheet" />
    <style>
        :root { --bg:#0f172a; --text:#e2e8f0; --muted:#94a3b8; --border:#334155; --primary: {{ $appBranding->primary_color ?? '#2563eb' }}; --accent: {{ $appBranding->accent_color ?? '#38bdf8' }}; --good:#22c55e; }
        * { box-sizing:border-box; margin:0; padding:0; }
        ...
        .filters { margin-top:10px; display:grid; grid-template-columns:1.4fr repeat(2,1fr) auto; gap:10px; align-items:end; }
        ...
        .progress-meta { display:flex; justify-content:space-between; color:var(--muted); font-size:.9rem; margin:8px 0; }
        .progress { height:14px; border-radius:999px; overflow:hidden; background:rgba(51,65,85,.55); border:1px solid var(--border); }
        .progress > span { display:block; height:100%; width:var(--progress, 0%); background:linear-gradient(90deg, var(--good), #38bdf8); }
        .pagination { margin-top:12px; }
        @media (max-width:1100px){ .filters{grid-template-columns:1fr;} .app-shell{grid-template-columns:1fr;} .sidebar{position:static; height:auto;} .main{padding-top:0;} }
...
<body>
    ...
    @endphp
    <div class="app-shell">
        @include('dashboard.partials.super-admin-sidebar', ['user' => $user, 'activePage' => 'completion'])

        <main class="main">
            <section class="topbar">
                <h1>All Student Completion Bars</h1>
                <p class="muted">Progress based on completed attendance days vs target days.</p>
            </section>

            <section class="card">
                <form method="GET" action="{{ route('dashboard.super-admin.completion') }}">
                    <div class="filters">
                        <div class="field">
                ...
                <div class="cards">
                    @forelse (($students ?? collect()) as $student)
                        <article class="student-card">
                            ...
                            <div class="muted" style="margin-top:6px;">PKL: {{ $student->pkl_start_date ?? '-' }} to {{ $student->pkl_end_date ?? '-' }}</div>
                            <div class="progress-meta">
                                <span>{{ (int) ($student->completed_days ?? 0) }} completed days</span>
                                <span>Target {{ (int) ($student->target_days ?? 0) }} days</span>
                            </div>
                            <div class="progress" style="--progress: {{ (int) ($student->progress_percent ?? 0) }}%;">
                                <span></span>
                            </div>
                            <div class="muted" style="margin-top:6px;">Progress: {{ (int) ($student->progress_percent ?? 0) }}%</div>
                        </article>
                    @empty
