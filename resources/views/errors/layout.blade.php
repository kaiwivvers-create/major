...
<head>
    ...
    <title>{{ ($title ?? 'Error') . ' - ' . ($appBranding->display_name ?? config('app.name', 'Laravel')) }}</title>
...
<div class="app-shell">
    <aside class="sidebar">
        @include('dashboard.partials.sidebar-brand', ['suffix' => $role === 'super_admin' ? 'Super Admin' : null])
        ...
        <nav class="sidebar-nav" aria-label="Error page menu">
            @if ($role === 'super_admin')
                ...
                @if ($canAccess('users_management', 'view'))
                    ...
                    <a href="{{ route('dashboard.super-admin.branding') }}">Branding</a>
